 ...
 * 【输出描述】 一行一个整数，表示最小的不平衡度。
 * 样例输入 5 3 3 5 5 2 5
 * 样例输出 2  提示 数据范围：1<=n<=100000, 1<=A[i]<=100000  输入样例1
 * 5 3
 * 3 5 5 2 5  输出样例1  2  解释  最终分为[3 5 5] [2] [5]，该种分段方式的不平衡度为2。
 */
public class Course2 {
    ...
    public static void main(String[] args) {
        ...
        int mindel = 0;
        int maxdel = Arrays.stream(num).max().getAsInt() - Arrays.stream(num).min().getAsInt();
        int left = mindel;
        int right = maxdel;
        while (left < right) {
            int mid = left + (right - left) / 2;
            int splits = split(num, mid);
            if (splits > k) {
                left = mid + 1;
            } else {
                right = mid;
    ...
    public static int split(int[] num, int maxdel) {
        int split = 1;
        int i = 0;
        int j = 1;
        while (j <= num.length) {
            int[] nownum = Arrays.copyOfRange(num, i, j);
            int nowdel = Arrays.stream(nownum).max().getAsInt() - Arrays.stream(nownum).min().getAsInt();
            if (nowdel > maxdel) {
                i = j - 1;
                j = i + 1;
                split++;
